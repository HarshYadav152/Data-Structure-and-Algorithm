...
struct node *bstSearch(struct node *root, int key)
{
    ...
    {
        return bstSearch(root->left, key); // apply same function for left sub tree
    }
    else // (key>root->data)
    {    
        return bstSearch(root->right, key); // apply same function for right sub tree
    }
}
